...
# pathological and rejected by the format check long before this).
json_escape() {
	printf '%s' "${1:?usage: json_escape <string>}" \
		| sed -e 's/\\/\\\\/g' -e 's/"/\\"/g'
}

# write_timeline_json: write <OUTPUT_DIR>/timeline.json — the manifest the
# in-map time-travel overlay (/overlay.js) fetches: newest-first list of
# {dir, save, date}. Regenerated by update_timeline_layer on every run
# (renders AND skips), atomically (temp + mv). Never fatal: a missing
# manifest only means the overlay stays hidden.
write_timeline_json() {
	local out="${OUTPUT_DIR}/timeline.json"
	local tmp="${out}.tmp.$$"

	if ! {
		printf '{"generated":"%s","renders":[' "$(date -u '+%Y-%m-%d %H:%M:%S UTC')"
 ...
 * date switcher for the current save; switching forwards the live query
 * string (x/y/z/s/layers kept by the viewer via history.replaceState), so
 * the view position survives the jump. Stays hidden when the manifest is
 * missing (e.g. a render dir served standalone) or the path is unknown. */
(function () {
	"use strict";
	...
	fetch("/timeline.json", { cache: "no-cache" })
		...
		.then(function (data) {
			...
			// The save to offer: parsed from the current render dir; on
			// /latest/ the newest entry's save (latest targets the newest).
			var save = currentDir
				? currentDir.slice(currentDir.indexOf("_") + 1)
				: renders[0].save;
			renders = renders.filter(function (e) { return e.save === save; });
